...
Page.getpath = (function(){
    ...
    if(location.port){
        return location.protocol +'//' + location.hostname +':'+ location.port+ '/component/pager';           
    }else{
        return location.protocol +'//' + location.hostname + '/component/pager';  
    }
}())
...
Page.prototype.view = function(){
    ...
    return '<div name="laypage'+ laypage.v +'" class="laypage_main laypageskin_'+ (conf.skin ? (function(skin){
        ...
        return conf.skip 
        ? '<span class="laypage_total"><label>共'+conf.pages+'页，</label><label>&#x5230;&#x7B2C;</label><input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>'

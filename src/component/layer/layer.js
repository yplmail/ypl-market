...
var $, win, ready = {
  getPath: function(){
    ...
    if(location.port){
        return location.protocol +'//' + location.hostname +':'+ location.port+ '/component/layer/';           
    }else{
        return location.protocol +'//' + location.hostname +'/component/layer/';  
    }
  }(),
...
var layer = {
  ...
  msg: function(content, options, end){ //最常用提示层
    ...
    return layer.open($.extend({
      content: content,
      time: (options && options.notimeout) ? 0 : 3000,
      shade: false,
      skin: skin,
      title: false,
      closeBtn: false,
  ...
  callback([
    ...
    '<div class="'+ doms[0] + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
      ...
      + '<span class="layui-layer-setwin">'+ function(){
        ...
        config.closeBtn && (closebtn += '<a class="layui-layer-ico '+ doms[7] +' '+ doms[7] + (config.title ? config.closeBtn : (config.type == 4 ? '1' : '3')) +'" href="javascript:;"></a>');
        return closebtn;
      }() + '</span>'
      + (config.btn ? function(){
        var button = '';
...
Class.pt.offset = function(){
  ...
  //修复IE8 loadding 不居中问题，add ypl
  if(/MSIE 8/.test(navigator.userAgent) && config.type == 3 && area[0] == win.width()){
       layero.width(config.maxWidth);
       area[0] = config.maxWidth;
  }  
